...
function createServerPermissionState(input: { sdk: ServerSDK; sync: ServerSync }) {
  ...
  function setGlobalAutoAccept(enabled: boolean) {
    if (meta.disposed) return
    setStore(
      produce((draft) => {
        draft.autoAccept[GLOBAL_ACCEPT_KEY] = enabled
      }),
    )
    if (!enabled) return
    // Immediately resolve permissions that are already waiting in synced sessions.
    for (const pending of Object.values(input.sync.session.data.permission)) {
      for (const permission of pending ?? []) {
        if (!permission?.id) continue
        void respondPending(permission, undefined, () => isGlobalAutoAccepting(store.autoAccept))
      }
    }
  }
