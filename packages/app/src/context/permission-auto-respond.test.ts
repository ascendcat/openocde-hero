...
describe("autoRespondsPermission", () => {
  ...
  test("falls back to the global auto-accept default", () => {
    const sessions = [session({ id: "root" })]
    const autoAccept = { [GLOBAL_ACCEPT_KEY]: true }

    expect(autoRespondsPermission(autoAccept, sessions, permission("root"), "/tmp/project")).toBe(true)
    expect(autoRespondsPermission(autoAccept, sessions, permission("root"))).toBe(true)
  })

  test("directory-level override takes precedence over the global default", () => {
    const directory = "/tmp/project"
    const sessions = [session({ id: "root" })]
    const autoAccept = {
      [GLOBAL_ACCEPT_KEY]: true,
      [`${base64Encode(directory)}/*`]: false,
    }
  ...
  test("session-level override takes precedence over the global default", () => {
    const directory = "/tmp/project"
    const sessions = [session({ id: "root" })]
    const autoAccept = {
      [GLOBAL_ACCEPT_KEY]: true,
  ...
  test("global default off keeps requiring approval", () => {
    const sessions = [session({ id: "root" })]

    expect(autoRespondsPermission({ [GLOBAL_ACCEPT_KEY]: false }, sessions, permission("root"), "/tmp/project")).toBe(
      false,
    )
  })
})
