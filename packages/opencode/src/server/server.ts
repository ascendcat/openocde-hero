...
type ListenOptions = CorsOptions & {
  ...
  mdns?: boolean
  mdnsDomain?: string
  // Secondary listeners (e.g. the desktop-managed web UI listener) must not
  // overwrite the module-global `url` that plugins use to reach the primary server.
  setUrl?: boolean
}
type ListenerState = {
  scope: Scope.Scope
...
interface ListenerServer {
  ...
  readonly node: import("node:http").Server
}
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    if (opts.setUrl !== false) url = listenerUrl

    return {
      hostname: opts.hostname,
      port: address.port,
      url: listenerUrl,
      node: state.http.node,
